...
let _config = {
  ...
  _rewindStartAt = (tween, totalTime, suppressEvents, force) =>
    tween._startAt &&
    (_reverting
      ? tween._startAt.revert(_revertConfig)
    ...
    if (!tl || (keyframes && !vars.stagger)) {
      ...
      if (prevStartAt) {
        time < 0 && runBackwards && immediateRender && !autoRevert
          ...
          : prevStartAt.revert(
              runBackwards && dur ? _revertConfig : _startAtRevertConfig
      ...
      if (startAt) {
        ...
        time < 0 &&
          ...
          tween._startAt.revert(_revertConfig); // rare edge case, like if a render is forced in the negative direction of a non-initted tween.
        if (immediateRender) {
          if (dur && time <= 0) {
  ...
  kill(revert, matchMedia) {
    if (revert) {
      ...
      this.getTweens()
        ...
        .sort((a, b) => b.g - a.g || -1)
        .forEach((o) => o.t.revert(revert)); // note: all of the _startAt tweens should be reverted in reverse order that thy were created, and they'll all have the same globalTime (-1) so the " || -1" in the sort keeps the order properly.
      this.data.forEach(
